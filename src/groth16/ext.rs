...
{
    let proofs =
        create_proof_batch_priority::<E, C, P>(vec![circuit], params, vec![r], vec![s], false, 0)?;
...
where
    ...
    C: Circuit<E> + Send,
{
    create_proof_batch_priority::<E, C, P>(circuits, params, r, s, false, 0)
}
...
{
    let proofs =
        create_proof_batch_priority::<E, C, P>(vec![circuit], params, vec![r], vec![s], true, 0)?;
    Ok(proofs.into_iter().next().unwrap())
}
...
where
    ...
    C: Circuit<E> + Send,
{
    create_proof_batch_priority::<E, C, P>(circuits, params, r, s, true, 0)
}
